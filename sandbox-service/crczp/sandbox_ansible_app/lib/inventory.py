...
class Inventory(BaseInventory):
    ...
    def _create_hosts(self) -> None:
        ...
        for node in self.topology_instance.get_nodes():
            ...
            # An image without cloud-init cannot receive the injected management key, so SSH
            # falls back to a password when the key is not accepted. sshpass must be present in
            # the Ansible runner image for this to work.
